...
const ServiceProviderLogin = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (server?.pendingApproval) {
        setPendingApproval(true);
        setError(server.message || 'Your account is pending admin approval.');
      } else {
        setError(server?.message || err.message || 'Login failed. Please try again.');
      }
      setSnackbarOpen(true);
    } finally {
      setLoading(false);
            ...
            />              <Box sx={{ display: 'flex', justifyContent: 'flex-end', mb: 2 }}>
              ...
              sx={{
                ...
                '&:hover': {
                  ...
                  boxShadow: '0 8px 25px rgba(15, 15, 55, 0.3)',
                },
                '&:disabled': {
                  bgcolor: '#B0BEC5',
                  color: '#FFFFFF',
